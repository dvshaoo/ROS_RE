...
# Field layouts cross-checked between the live DataType object and the XML.
# DTS_APPEARANCE_PACKAGE3 has FOUR fields (itemList ARRAY, itemsList ARRAY,
# layoutInfo PY_DICT, _packageCapacity UINT32) = 15 bytes. An earlier attempt read the
# object's vector at +0xe0 (15 entries = 3 records) and produced an 11-byte encoding;
# that vector is not the field list. The correct one is at +0x20 (20 entries = 4
# records, DataType pointer at qword 3 of each 5-qword record), and it agrees with the
# XML. Feeding 11 bytes desynced the client mid-dict at offset 312:
#   SequenceDataType::createFromStream: Invalid size on stream: 3034626
# rankRecord / braveBookDataList really are single-ARRAY dicts (their +0xe0 vectors
# describe their ELEMENT types, not their own fields).
FIXED_DICT_RUNTIME = {
    'dtsAppearancePackage': (PRIM['ARRAY'] + PRIM['ARRAY']
                             + PRIM['PYTHON'] + PRIM['UINT32']),
    'rankRecord': PRIM['ARRAY'],
    'braveBookDataList': PRIM['ARRAY'],
}
...
for ordinal, r in enumerate(inc):
    ...
    if r['name'] == TARGET:
        ...
        # Field lists read from the runtime DataType object (scratch/fixed_dict_fields.py),
        # which disagrees with the XML. rankRecord / braveBookDataList really are
        # single-ARRAY dicts (the multi-field layouts seen at +0xe0 belong to their
        # ELEMENT types), but dtsAppearancePackage genuinely has three fields.
        blob = FIXED_DICT_RUNTIME.get(r['name'])
        if blob is None:
            xml_t = str(r['type'] or '')
