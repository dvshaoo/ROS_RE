...
def find_method_containing(target_addr):
    for c_i in range(class_defs_size):
        ...
        for _ in range(dm + vm):
            ...
            if code_off:
                ...
                if insns_start <= target_addr < insns_end:
                    return get_type(class_idx), get_method(m_idx), code_off, insns_start, insns_end
    return None

targets = [0x3e0554, 0x3cc968, 0x3ff902, 0x3ff958, 0x3ff978]
for t in targets:
    res = find_method_containing(t)
    if res:
        cname, mname, code_off, start, end = res
        print(f"Address 0x{t:x} -> Class: {cname}, Method: {mname}, code_off: 0x{code_off:x} [0x{start:x} - 0x{end:x}]")
    else:
        print(f"Address 0x{t:x} -> Not found in code ranges")
